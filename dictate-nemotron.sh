...
# local-llm dictation script using Nemotron 3.5 ASR via parakeet.cpp with Vulkan GPU acceleration.
# Dependencies: arecord, parakeet-cli, xdotool, xclip, notify-send

set -euo pipefail

# Configurations
PARAKEET_DIR="/home/dpavlin/local-llm/parakeet.cpp"
PARAKEET_CLI="${PARAKEET_DIR}/build/examples/cli/parakeet-cli"
MODEL_PATH="/home/dpavlin/local-llm/models/nemotron-3.5-asr-streaming-0.6b-q4_k.gguf"
...
verify_dependencies() {
    ...
    if [[ ! -f "$PARAKEET_CLI" ]]; then
        missing+=("parakeet-cli (at $PARAKEET_CLI)")
    fi
    
    if [[ ! -f "$MODEL_PATH" ]]; then
        missing+=("nemotron model (at $MODEL_PATH)")
    fi
...
start_recording() {
    ...
    # 3. Record at 16kHz mono 16-bit PCM (native format) using ffmpeg
...
stop_recording_and_transcribe() {
    ...
    play_sound stop
    notify "Dictation" "Transcribing audio using Nemotron..."
    ...
    # Run parakeet-cli and capture text, silencing the Vulkan logs to stderr
    local text
    text=$("$PARAKEET_CLI" transcribe --model "$MODEL_PATH" --input "$AUDIO_FILE" 2>/dev/null || true)
    
    # Clean up the output text (strip <locale> tags and normalize whitespace)
    text=$(echo "$text" | sed -E -e 's/<[a-zA-Z-]{2,5}>//g' -e 's/[[:space:]]+/ /g' -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//')
